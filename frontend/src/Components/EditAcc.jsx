...
function EditAcc() {
  ...
  useEffect(() => {
    ...
    const fetchAccountInfo = async () => {
      const data = await fetchWithAuth("http://localhost:5000/api/profile", options);
      setAccountInfo(data);
...
console.log(accountInfo);
  const handleSubmit = async (e) => {
    ...
    const response = await fetchWithAuth("http://localhost:5000/api/profile/edit", options2);
      navigate("/profile");
